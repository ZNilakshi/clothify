...
public class OrderController {
    ...
    // PUT /api/orders/{id}/tracking - Add tracking details
    @PutMapping("/{id}/tracking")
    public ResponseEntity<OrderDTO> addTrackingDetails(
            @PathVariable Long id,
            @RequestBody TrackingDTO trackingDTO) {
        log.info("PUT /api/orders/{}/tracking", id);
        OrderDTO order = orderService.addTrackingDetails(id, trackingDTO);
        return ResponseEntity.ok(order);
    }
}