...
public class SmsServiceImpl implements SmsService {
    ...
    @Override
    @Async
    public void sendOrderConfirmationSms(Long orderId, String phoneNumber, BigDecimal totalAmount) {
        try {
            String message = String.format(
                    "%s: Order #%d confirmed! Total: Rs %.2f. Thank you for shopping with us!",
                    appName,
                    orderId,
                    totalAmount
            );
    ...
    @Override
    @Async
    public void sendOrderStatusUpdateSms(Long orderId, String phoneNumber, String oldStatus, String newStatus) {
        try {
            String message = String.format(
                    "%s: Order #%d status updated: %s → %s.",
                    appName,
                    orderId,
                    oldStatus,
                    newStatus
